...
<div class="container mt-5">
    <div class="card border-custom">
        <form action="{{ route('discounts.update', $discount->id) }}" method="POST" enctype="multipart/form-data"
            ...
            @csrf
            <div class="card-head">
                <h1 class="text-center text-primary">Edit Discount</h1>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-select" data-control="select2" data-placeholder="Select Type" required
                                ...
                                <option value="{{ $type }}" {{ $discount->type==$type ? 'selected' : '' }}>{{
                                    ucwords($type)
                                    }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" step="any" min="0" class="form-control" name="value"
                                placeholder="Enter Value..." value="{{ $discount->value }}" required />
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control" rows="3"
                                placeholder="Enter Description...">{{ $discount->description }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
